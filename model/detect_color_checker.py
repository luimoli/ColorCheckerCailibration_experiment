...
def is_square(contour, tolerance=0.001):
...
def find_contour(image):
    ...
    for contour in contours:
        ...
        if 10000 < cv2.contourArea(curve) < 100000 and is_square(curve):
...
def detect_color_checker(image):
    ...
    for centroid in centroid_array:
        mean_value = image[centroid[1] - 20:centroid[1] + 20, centroid[0] - 20:centroid[0] + 20].mean(axis=(0, 1))
    ...
    return sorted_centroid, clusters
...
if __name__ == '__main__':
    ...
    sorted_centroid, clusters = detect_color_checker(image_rgb)
    
    index = np.int32(sorted_centroid)
    x, y = index[:, 0], index[:, 1]
    checker_arr =  image_rgb[x, y]
    print('.')
    # image_rgb[x, y]